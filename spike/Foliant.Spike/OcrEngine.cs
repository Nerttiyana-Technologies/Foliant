// Stage 3 — PaddleOCR ONNX: DB text detection + CTC recognition.
// Models: monkt/paddleocr-onnx (PP-OCRv5 server det + en mobile rec, h=32).
//
// Spike simplifications vs full PaddleOCR (documented in spike/NOTES.md):
...
//  - Recognition runs one line at a time (no width-bucketed batching). Slow is fine.
...
public sealed record TextLine(float X1, float Y1, float X2, float Y2, string Text, float Confidence);
...
public sealed class PaddleOcrEngine : IDisposable
{
    ...
    public IReadOnlyList<TextLine> Recognize(SKBitmap page)
    {
        var boxes = DetectTextBoxes(page);
        var lines = new List<TextLine>();
        foreach (var box in boxes)
        {
            var (text, conf) = RecognizeLine(page, box);
            if (text.Length > 0)
                lines.Add(new TextLine(box.Left, box.Top, box.Right, box.Bottom, text, conf));
        }
        // top-to-bottom, then left-to-right (crude; stage 5 does real reading order)
        return lines
            .OrderBy(l => Math.Round(l.Y1 / 20.0))
            .ThenBy(l => l.X1)
            .ToList();
    }
    ...
    // Ratio is judged against TRUE ink height (DB unclip inflates the box).
    ...
    private static List<(int Start, int Width, bool SpaceBefore)> SplitAtWhitespace(SKBitmap crop, int maxChunkWidth)
    {
        int w = crop.Width, h = crop.Height;
        var px = crop.Pixels;
